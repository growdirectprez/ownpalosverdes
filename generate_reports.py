...
def render_report(year, month, stats, city_stats, prev_stats, all_months, sales):
    ...
    if current_key in PRELIMINARY_MONTHS:
        prelim_banner = (
            '<div style="max-width:760px;margin:24px auto 48px;padding:16px 20px;'
...
{{
  ...
  "itemListElement": [
    ...
    {{ "@type": "ListItem", "position": 2, "name": "Market Reports", "item": "{SITE_URL}/market/" }},
    {{ "@type": "ListItem", "position": 3, "name": "{month_name} {year}", "item": "{SITE_URL}/market/{year}-{month:02d}/" }}
  ]
}}
</script>
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .cta-sub {{ font-family: var(--sans); font-size: 0.85rem; color: var(--drift); margin-top: -0.6rem; margin-bottom: 1.25rem; }}
  .report-lead {{ max-width: 460px; margin: 0 auto; display: flex; flex-direction: column; gap: 10px; text-align: left; }}
  .report-lead input {{ padding: 12px 14px; border: 1px solid var(--sand-mid); background: var(--white); font-family: var(--sans); font-size: 0.9rem; color: var(--ink); }}
  .report-lead input:focus {{ outline: none; border-color: var(--copper); }}
  .report-lead button {{ margin-top: 4px; padding: 13px 28px; background: var(--ink); color: var(--white); border: none; font-family: var(--sans); font-size: 0.72rem; letter-spacing: 0.1em; text-transform: uppercase; cursor: pointer; transition: background 0.2s; }}
  .report-lead button:hover {{ background: var(--copper); }}
  .report-lead-msg {{ display: none; font-family: var(--serif); color: var(--teal); font-size: 1.05rem; margin-top: 0.5rem; text-align: center; }}
  .related-guides {{ max-width: 1100px; margin: 3.5rem auto 0; padding: 0 2rem; }}
  .guide-cards {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1rem; margin-top: 1.5rem; }}
  .guide-card {{ display: block; padding: 1.5rem; background: var(--white); border: 1px solid var(--sand-mid); text-decoration: none; transition: border-color 0.2s, transform 0.2s; }}
  .guide-card:hover {{ border-color: var(--copper); transform: translateY(-2px); }}
  .guide-card strong {{ display: block; font-family: var(--serif); font-size: 1.15rem; color: var(--ink); margin-bottom: 0.4rem; }}
  .guide-card span {{ font-size: 0.85rem; color: var(--drift); line-height: 1.4; }}
...
<section class="related-guides">
  ...
  <h2>Get to know <em>the Peninsula.</em></h2>
  <div class="guide-cards">
    <a href="/guides/neighborhoods/" class="guide-card">
      <strong>Neighborhood Guide</strong>
      <span>All 12 Palos Verdes neighborhoods, street by street.</span>
    </a>
    <a href="/guides/schools/" class="guide-card">
      <strong>PVPUSD School Guide</strong>
      <span>Boundaries, open choice, permits &amp; private options.</span>
    </a>
    <a href="/guides/lifestyle/" class="guide-card">
...
<section class="cta-bar">
  ...
  <p class="cta-sub">Get a personalized analysis from Angelique &mdash; no obligation.</p>
  <a href="https://angeliquelyle.com/contact?utm_source=ownpalosverdes&utm_medium=market_report&utm_campaign={year}-{month:02d}" target="_blank" rel="noopener" onclick="if(typeof gtag==='function'){{gtag('event','generate_lead',{{form_type:'market_{year}-{month:02d}',destination:'angeliquelyle.com/contact'}});}}">Talk to Angelique &rarr;</a>
</section>
...
<footer>
  <img src="/images/logos/realtor-eho-ink.svg" alt="REALTOR&#174; and Equal Housing Opportunity" style="height:44px;width:auto;opacity:0.85;margin-bottom:0.9rem;">
